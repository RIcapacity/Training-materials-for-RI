# Welcome to Training-materials-for-RI
welcome to the reporisoty of training materials for professional skills needed by research infrastructure scientists. 

## How to contribute
We encourage contributions! You can:  
1. Suggest improvements by opening an **Issue**.  
2. Add content by **forking** the repository, making changes, and submitting a **Pull Request**.  
3. Join discussions to help improve the materials.  
